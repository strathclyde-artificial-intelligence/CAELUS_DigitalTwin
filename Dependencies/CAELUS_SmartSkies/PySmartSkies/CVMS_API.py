...
class CVMS_API():
    ...
    @staticmethod
    def __place_order(session, buyer: Vendor, seller: Vendor, products: List[Product]):
    ...
    def __process_auth_payload(self, payload):
        try:
            if payload is None or 'result' not in payload or payload['result'] != True:
                raise Exception('Error while authenticating CVMS user.')
            bearer = payload['data']['authToken']
            customer = Customer(payload['data']['user'])
            self._logged_customer = customer
            self._session.store_cvms_bearer(bearer)
        except Exception as e:
            self._logger.error(f'Malformed authentication payload.')
    ...
    def place_order(self, seller: Vendor, products: List[Product]):
    ...
    def checkout_orders(self, orders: List[Order]):
        if orders is None:
